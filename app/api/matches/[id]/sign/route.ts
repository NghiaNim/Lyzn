...
export async function POST(
  ...
  try {
    ...
    // Find the negotiation for this terms hash
    ...
    if (!negotiation) {
      return NextResponse.json(
        { error: 'Negotiation not found for this terms hash' },
        { status: 404 }
      );
    }
    ...
    const hasPartyASignature = signatures.some((s: any) => s.userId === partyAId);
    const hasPartyBSignature = signatures.some((s: any) => s.userId === partyBId);
