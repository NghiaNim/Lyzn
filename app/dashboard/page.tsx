      ...
      <div className="relative pt-24 pb-12 px-4 sm:px-6 z-10">
        <div className="max-w-7xl mx-auto">
          <div className="mb-6 sm:mb-8">
            ...
            <p className="text-sm sm:text-base text-gray-400">Monitor your active positions and hedges</p>
          </div>
          ...
          {positions.length === 0 ? (
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-6 sm:mb-8">
                <div className="card bg-slate-900/50 backdrop-blur-sm border-slate-800 p-4">
                  <p className="text-xs sm:text-sm text-gray-400 mb-1">Total Invested</p>
                  <p className="text-xl sm:text-2xl font-bold">${totalInvested.toLocaleString()}</p>
                  <p className="text-xs text-gray-500 mt-1">{positions.length} position{positions.length !== 1 ? 's' : ''}</p>
                </div>
                <div className="card bg-slate-900/50 backdrop-blur-sm border-slate-800 p-4">
                  <p className="text-xs sm:text-sm text-gray-400 mb-1">Potential Payout</p>
                  <p className="text-xl sm:text-2xl font-bold text-green-400">${totalPotentialPayout.toLocaleString()}</p>
                  <p className="text-xs text-gray-500 mt-1">If all conditions met</p>
                </div>
                <div className="card bg-slate-900/50 backdrop-blur-sm border-slate-800 p-4">
                  <p className="text-xs sm:text-sm text-gray-400 mb-1">Potential Profit</p>
                  <p className="text-xl sm:text-2xl font-bold text-green-400">+${potentialProfit.toLocaleString()}</p>
              ...
              <div className="mb-8">
                ...
                <div className="space-y-4">
                  {positions.map((position) => {
                    ...
                    return (
                      <div key={position.id + position.purchasedAt} className="card bg-slate-900/50 backdrop-blur-sm border-slate-800 hover:border-blue-500 transition-colors p-4 sm:p-6">
                        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-4 gap-3">
                          <div className="flex-1">
                            <h3 className="text-base sm:text-xl font-semibold mb-2">{position.title}</h3>
                          ...
                          <div className="sm:ml-4">
                            <span className="px-3 py-1 bg-blue-600/20 text-blue-400 rounded text-xs sm:text-sm inline-block">
                              Active
                            </span>
                          </div>
                        </div>

                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 py-3 border-y border-gray-700">
                          <div>
                            <p className="text-xs text-gray-400 mb-1">Your Cost</p>
                            <p className="text-sm sm:text-base font-semibold">${position.cost.toLocaleString()}</p>
                          </div>
                          <div>
                            <p className="text-xs text-gray-400 mb-1">Potential Payout</p>
                            <p className="text-sm sm:text-base font-semibold text-green-400">${position.payout.toLocaleString()}</p>
                          </div>
                          <div>
                            <p className="text-xs text-gray-400 mb-1">Potential Profit</p>
                            <p className="text-sm sm:text-base font-semibold text-green-400">+${profit.toLocaleString()}</p>
                          </div>
                          <div>
                            <p className="text-xs text-gray-400 mb-1">ROI if Won</p>
                            <p className="text-sm sm:text-base font-semibold text-green-400">{roi}%</p>
                          </div>
                        </div>
