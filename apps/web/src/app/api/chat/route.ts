...
export async function POST(request: NextRequest) {
  try {
    ...
    const { messages, businessContext } = body as {
      messages: Message[];
      businessContext?: any;
    };
    ...
    if (!process.env.OPENAI_API_KEY) {
      ...
      return useFallbackResponse(messages);
    }
...
// Fallback for when Grok API is not available
function useFallbackResponse(messages: Message[]) {
